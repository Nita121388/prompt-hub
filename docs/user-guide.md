...
- 提取 emoji 🖨️ 并预填充（如果存在）
- 移除标记行，仅保留正文内容

**标记格式说明**：
- 支持两种格式：
  - 专用标记：`# prompt: 名称`（优先级更高）
  - 标准 Markdown：`# 名称`
...
- **单击**：复制到剪贴板
- **双击**：打开编辑器
- **鼠标悬停**：显示快速操作按钮（inline buttons）

**快速操作按钮**（鼠标悬停时显示在每行右侧）：
- **📋 复制**：一键复制 Prompt 内容到剪贴板
- **✏️ 编辑**：直接打开源文件进行编辑
- **✨ 优化**：使用 AI 优化 Prompt 内容
- **🗑️ 删除**：删除该 Prompt
...
- **🔄 刷新**：重新加载列表
- **⚙️ 设置**：快速打开插件设置
- **📂 打开仓库文件夹**：在系统文件管理器中打开 Prompt 存储目录
- **📝 新建 Prompt**：创建空白 Prompt 文件
- **🔀 Git 同步**（如启用 Git）：执行 Git 同步
...
**功能说明**：
- 自动解析配置的存储路径（支持 `~`、`${workspaceFolder}` 等变量）
- 跨平台兼容（Windows、macOS、Linux）
- **直接打开文件夹内部**，而不是在父目录中选中它
- Windows: 在资源管理器中打开文件夹
- macOS: 在 Finder 中打开文件夹
- Linux: 在默认文件管理器中打开文件夹
