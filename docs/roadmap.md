...
**更新时间**：2025-11-11（本文件由 `PROGRESS.md` 整理更名而来）
**项目版本**：v0.1.0-dev
**总体完成度**：约 50-60%
...
Otter 是一个 VSCode 扩展，用于统一管理和组织 AI Prompt，支持本地存储、Git 同步、AI 辅助生成。当前已完成核心框架和基础功能，但仍有关键模块（测试、首次引导、Git 同步完善）待完成。
...
- ❌ `otter.importJSON` - 从 JSON 文件导入 Prompt
- ❌ `otter.exportJSON` - 导出所有 Prompt 为 JSON
- ❌ `otter.exportToMarkdown` - 批量导出 Markdown
...
- ❌ 智能配置问题检测（状态栏警告）
- ❌ 配置快捷跳转命令：
  - `otter.configureStorage` - 直接跳转存储配置
  - `otter.configureGit` - 直接跳转 Git 配置
  - `otter.configureAI` - 直接跳转 AI 配置
...
- ❌ `otter.viewLogs` - 查看 Output Channel 日志
- ❌ `otter.clearCache` - 清除缓存数据
- ❌ `otter.ai.viewUsage` - AI 使用记录详细视图
